...
# Create wrapper script for libreoffice command to set LD_LIBRARY_PATH
mv /usr/bin/libreoffice /usr/bin/libreoffice.bin
cat > /usr/bin/libreoffice << 'EOF'
#!/bin/bash
export LD_LIBRARY_PATH=:/usr/lib/libreoffice/program:/usr/lib/$(arch)-linux-gnu/
exec /usr/bin/libreoffice.bin "$@"
EOF
